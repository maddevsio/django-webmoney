...
class PrerequestForm(ExtraPaymentForm):
    ...
    LMI_PAYEE_PURSE = forms.RegexField(regex=PURSE_RE, required=False)

    LMI_PAYMENT_AMOUNT = forms.DecimalField(
        max_digits=7, decimal_places=2, label=_(u'Amount'), required=False)
    LMI_MODE = forms.IntegerField(
        label=_('Test mode'), min_value=0, max_value=1, required=False)

    LMI_PAYER_WM = forms.RegexField(regex=WMID_RE, required=False)
    LMI_PAYER_PURSE = forms.RegexField(regex=PURSE_RE, required=False)
